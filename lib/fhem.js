...
FHEMDevice(platform, s) {
    ...
    if (match = s.PossibleSets.match(/(^| )rgb:colorpicker/)) {
        ...
        if (s.Internals.TYPE === 'EspLedController') {
            // TODO: there is maybe a better way than providing custom homekit functions for the conversion?
            this.mappings.ColorMode = {reading: 'sat', valueCt: '0'};
            this.mappings.ColorMode.reading2homekit = function (mapping, orig) {
                return parseInt(orig);
            }.bind(null, this.mappings.ColorMode);
            this.mappings.ColorMode.homekit2reading = function (mapping, orig) {
                return parseInt(orig);
            }.bind(null, this.mappings.ColorMode);

            this.mappings.ColorTemperature = {reading: 'ct', cmd: 'white'};
...
//log( util.inspect(s) );
    ...
    // get alias using this priority: ghomeName -> assistantName -> alias -> NAME
    this.alias = s.Attributes.ghomeName ? s.Attributes.ghomeName : s.Attributes.assistantName ? s.Attributes.assistantName : s.Attributes.alias ? s.Attributes.alias : s.Internals.NAME;
